...
from electrum_pkb.plugins import BasePlugin, hook
from electrum_pkb.i18n import _
from electrum_pkb_gui.qt.util import *
from electrum_pkb_gui.qt.amountedit import AmountEdit
...
class Exchanger(threading.Thread):
    ...
    def update_b2c(self):
        jsonresp = self.get_json('www.bit2c.co.il', "/Exchanges/PKBNIS/Ticker.json")
        return {"NIS": Decimal(str(jsonresp["ll"]))}

    def update_bv(self):
        jsonresp = self.get_json('api.bitcoinvenezuela.com', "/")
        return dict([(r, Decimal(jsonresp["PKB"][r])) for r in jsonresp["PKB"]])

    def update_bf(self):
        jsonresp = self.get_json('api.bitfinex.com', "/v1/pubticker/pkbusd")
        return {"USD": Decimal(jsonresp["last_price"])}

    def update_be(self):
        quote_currencies = {"CNH": 0.0, "EUR": 0.0, "GBP": 0.0, "RUR": 0.0, "USD": 0.0}
        jsonresp = self.get_json('btc-e.com', "/api/3/ticker/" + ('-'.join(['pkb_'+c.lower() for c in quote_currencies])))
        for cur in quote_currencies:
            quote_currencies[cur] = Decimal(str(jsonresp['pkb_'+cur.lower()]["last"]))
        return quote_currencies

    def update_cv(self):
        jsonresp = self.get_json('www.cavirtex.com', "/api2/ticker.json?currencypair=PKBCAD")
        cadprice = jsonresp["ticker"]["PKBCAD"]["last"]
        return {"CAD": Decimal(str(cadprice))}

    def update_CNY(self):
        jsonresp = self.get_json('data.btcchina.com', "/data/ticker?market=pkbcny")
    ...
    def update_gc(self):
        ...
        quote_currencies = {}
        for r in jsonresp["prices"]["PKB"]:
            quote_currencies[r] = Decimal(jsonresp["prices"]["PKB"][r])
        return quote_currencies

    def update_hb(self):
        quote_currencies = {"EUR": 0.0, "USD": 0.0}
        for cur in quote_currencies:
            quote_currencies[cur] = Decimal(str(self.get_json('api.hitbtc.com', "/api/1/public/PKB" + cur + "/ticker")["last"]))
        return quote_currencies

    def update_kk(self):
        resp_currencies = self.get_json('api.kraken.com', "/0/public/AssetPairs")["result"]
        pairs = ','.join([k for k in resp_currencies if k.startswith("XPKBZ")])
    ...
    def update_ok(self):
        jsonresp = self.get_json('www.okcoin.cn', "/api/ticker.do?symbol=pkb_cny")
        cnyprice = jsonresp["ticker"]["last"]
        return {"CNY": Decimal(str(cnyprice))}
    ...
    def request_history_rates(self):
        ...
        if self.cur_exchange == "CoinDesk":
            ...
            if cur_currency in ("ARS", "EUR", "USD", "VEF"):
                try:
                    self.resp_hist = self.exchanger.get_json('api.bitcoinvenezuela.com', "/historical/index.php?coin=PKB")[cur_currency + '_PKB']
                except Exception:
                    return
            else:
                return
            ...
            if cur_currency in ("EUR", "USD"):
                try:
                    self.resp_hist = self.exchanger.get_json('api.kraken.com', "https://api.kraken.com/0/public/OHLC?pair=PKB"+cur_currency+"&interval=1440")['result']['XPKBZ'+cur_currency]
