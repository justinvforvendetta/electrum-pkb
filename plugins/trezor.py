...
from electrum_pkb.account import BIP32_Account
from electrum_pkb.bitcoin import EncodeBase58Check, public_key_to_bc_address, bc_address_to_hash_160
from electrum_pkb.i18n import _
from electrum_pkb.plugins import BasePlugin, hook, always_hook, run_hook
from electrum_pkb.transaction import deserialize
from electrum_pkb.wallet import BIP32_HD_Wallet
from electrum_pkb.util import print_error
from electrum_pkb.wallet import pw_decode, bip32_private_derivation, bip32_root
...
class TrezorWallet(BIP32_HD_Wallet):
    ...
    def show_address(self, address):
        ...
        try:
            ...
            give_error(e)
        try:
            self.get_client().get_address('ParkByte', address_n, True)
        except Exception, e:
            give_error(e)
        finally:
    ...
    def sign_message(self, address, message, password):
        ...
        try:
            ...
            give_error(e)
        try:
            msg_sig = self.get_client().sign_message('ParkByte', address_n, message)
        except Exception, e:
            give_error(e)
        finally:
            self.plugin.handler.stop()
    ...
    def sign_transaction(self, tx, password):
        ...
        try:
            signed_tx = self.get_client().sign_tx('ParkByte', inputs, outputs)[1]
        except Exception, e:
            give_error(e)
        finally:
            self.plugin.handler.stop()
...
class TrezorQtHandler:
    ...
    def passphrase_dialog(self):
        from electrum_pkb_gui.qt.password_dialog import make_password_dialog, run_password_dialog
        d = QDialog()
        d.setModal(1)
