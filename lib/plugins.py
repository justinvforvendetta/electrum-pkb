...
def init_plugins(config, is_local, is_gui):
    ...
    if is_local:
        ...
        electrum_plugins = imp.load_module('electrum_pkb_plugins', fp, pathname, description)
        loader = lambda name: imp.load_source('electrum_pkb_plugins.' + name, os.path.join(pathname, name + '.py'))
    else:
        electrum_plugins = __import__('electrum_pkb_plugins')
        loader = lambda name: __import__('electrum_pkb_plugins.' + name, fromlist=['electrum_pkb_plugins'])
