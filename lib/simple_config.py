...
SYSTEM_CONFIG_PATH = "/etc/electrum-pkb.conf"
...
class SimpleConfig(object):
    ...
    def set_key(self, key, value, save = True):
        if not self.is_modifiable(key):
            print "Warning: not changing key '%s' because it is not modifiable" \
                  " (passed as command line option or defined in /etc/electrum-pkb.conf)"%key
            return
    ...
    def get_wallet_path(self):
        ...
        old_path = os.path.join(self.path, "electrum-pkb.dat")
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-pkb.conf."""
    result = {}
    if os.path.exists(path):
        try:
            import ConfigParser
        except ImportError:
            print "cannot parse electrum-pkb.conf. please install ConfigParser"
            return
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-pkb.conf into user_config[]."""
    if not path:
        return {}
