...
from electrum_pkb.i18n import _, set_language
from electrum_pkb.util import block_explorer, block_explorer_info, block_explorer_URL
...
from electrum_pkb.bitcoin import MIN_RELAY_TX_FEE, is_valid
from electrum_pkb.plugins import run_hook

import icons_rc

from electrum_pkb.util import format_satoshis, format_time, NotEnoughFunds, StoreDict
from electrum_pkb import Transaction
from electrum_pkb import mnemonic
from electrum_pkb import util, bitcoin, commands, Wallet
from electrum_pkb import SimpleConfig, Wallet, WalletStorage
from electrum_pkb import Imported_Wallet
...
from electrum_pkb import ELECTRUM_VERSION
import re
...
from electrum_pkb.paymentrequest import PR_UNPAID, PR_PAID, PR_EXPIRED
from electrum_pkb.paymentrequest import PaymentRequest, InvoiceStore, get_payment_request, make_payment_request
...
class ElectrumWindow(QMainWindow):
    ...
    def __init__(self, config, network, gui_object):
        ...
        self.setWindowIcon(QIcon(":icons/electrum-pkb.png"))
        self.init_menubar()
    ...
    def load_wallet(self, wallet):
        import electrum_pkb as electrum
        self.wallet = wallet
        ...
        title = 'Electrum-PKB %s  -  %s' % (self.wallet.electrum_version, self.wallet.basename())
    ...
    def init_menubar(self):
        ...
        help_menu.addAction(_("&Official website"), lambda: webbrowser.open("http://electrum-pkb.space"))
        help_menu.addSeparator()
        help_menu.addAction(_("&Documentation"), lambda: webbrowser.open("http://electrum-pkb.space")).setShortcut(QKeySequence.HelpContents)
        help_menu.addAction(_("&Report Bug"), self.show_report_bug)

        self.setMenuBar(menubar)

    def show_about(self):
        QMessageBox.about(self, "Electrum-PKB",
    ...
    def show_report_bug(self):
        QMessageBox.information(self, "Electrum-PKB - " + _("Reporting Bugs"),
            _("Please report any bugs as issues on github:")+" <a href=\"https://github.com/pkb/electrum-pkb/issues\">https://github.com/pkb/electrum-pkb/issues</a>")
    ...
    def notify(self, message):
        if self.tray:
            self.tray.showMessage("Electrum-PKB", message, QSystemTrayIcon.Information, 20000)
    ...
    def read_send_tab(self):
        ...
        for _type, addr, amount in outputs:
            if addr is None:
                QMessageBox.warning(self, _('Error'), _('ParkByte Address is None'), _('OK'))
                return
            if _type == 'address' and not bitcoin.is_address(addr):
                QMessageBox.warning(self, _('Error'), _('Invalid ParkByte Address'), _('OK'))
                return
            if amount is None:
    ...
    def pay_from_URI(self,URI):
        ...
        try:
            ...
            QMessageBox.warning(self, _('Error'), _('Invalid parkbyte URI:') + '\n' + str(e), _('OK'))
            return
    ...
    def new_account_dialog(self, password):
        ...
        e = QLineEdit()
        vbox.addWidget(e)
        msg = _("Note: Newly created accounts are 'pending' until they receive parkbytes.") + " " \
    ...
    def read_tx_from_qrcode(self):
        from electrum_pkb import qrscanner
        try:
    ...
    def export_privkeys_dialog(self, password):
        ...
        defaultname = 'electrum-pkb-private-keys.csv'
    ...
    def do_export_labels(self):
        ...
        try:
            fileName = self.getSaveFileName(_("Select file to save your labels"), 'electrum-pkb_labels.dat', "*.dat")
            if fileName:
                with open(fileName, 'w+') as f:
                    json.dump(labels, f)
    ...
    def export_history_dialog(self):
        ...
        vbox = QVBoxLayout(d)
        defaultname = os.path.expanduser('~/electrum-pkb-history.csv')
    ...
    def settings_dialog(self):
        ...
        from electrum_pkb.i18n import languages
    ...
    def plugins_dialog(self):
        from electrum_pkb.plugins import plugins, descriptions, is_available, loader
