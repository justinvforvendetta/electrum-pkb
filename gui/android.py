...
from electrum_pkb import SimpleConfig, Wallet, WalletStorage, format_satoshis
from electrum_pkb.bitcoin import is_address
...
def pay_to(recipient, amount, label):
    ...
    droid.dialogCreateSpinnerProgress("Electrum-PKB", "signing transaction...")
    droid.dialogShow()
...
def receive_loop():
    ...
    while out is None:
        ...
        if not event:
            ...
            amount = modal_input('Amount', 'Amount you want to receive (in PKB). ', format_satoshis(receive_amount) if receive_amount else None, "numberDecimal")
...
class ElectrumGui:
    ...
    def __init__(self, config, _network):
        ...
        if not storage.file_exists:
            ...
            droid.dialogCreateSpinnerProgress("Electrum-PKB", msg)
            droid.dialogShow()
